...
async function analyzeAll() {
  ...
  for (const sym of toAnalyze) {
    ...
    if (dec && dec.verdict!=='ERROR' && dec.verdict!=='THINKING') {
      ...
      if (age < 300000) continue; // Skip if fresh decision under 5 min
    }
    // AI #1 first
    await askAI(sym, 'primary');
    await sleep(400);
...
// ── SCHEDULES ──
setInterval(fetchAlpacaPrices, 5000);                     // Real prices every 5s (always on)
setInterval(()=>{ if(!isWeekend()) analyzeAll(); }, 600000);         // Smart dual AI — weekdays only
setInterval(()=>{ if(!isWeekend()) rotateRoster(); }, 300000);       // Rotate — weekdays only
setInterval(()=>{ if(!isWeekend()) detectMarketRegime(); }, 300000); // Regime — weekdays only
setInterval(()=>{ if(!isWeekend()) scanMarket('full'); }, 600000);   // Full scan — weekdays only
setInterval(()=>{ if(!isWeekend()) fetchSentiment(); }, 300000);     // News — weekdays only
setInterval(()=>{ checkDrawdown(); calcPortfolioHeat(); }, 60000);   // Safety checks always on
...
// Weekend sleep — notify Friday close
cron.schedule('0 17 * * 5', async()=>{
  await sendDiscordAlert('😴 **BRAIN GOING TO SLEEP**\nMarkets closed for the weekend\nAll AI calls paused — resuming Monday 4am ET\nQuantum is still running crypto 24/7');
}, {timezone:'America/New_York'});

// Monday wake up
cron.schedule('55 3 * * 1', async()=>{
  await sendDiscordAlert('⏰ **BRAIN WAKING UP**\nNew week — markets open soon\nScanning for opportunities...');
}, {timezone:'America/New_York'});
