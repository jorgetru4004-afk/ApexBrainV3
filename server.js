...
async function askAI(sym, role='primary') {
  ...
  const rolePrompt = role === 'secondary'
    ...
    : `You are AI ANALYST #1 — elite penny stock trader. Find the best setups.`;
...
async function analyzeAll() {
  ...
  console.log(`🧠 Analyzing ${toAnalyze.length} tickers (dual AI)...`);
  for (const sym of toAnalyze) {
    // Skip if fresh decision under 4 min
...
async function scanMarket(scanType) {
  ...
  const scanFocus = {
    full:     `top 5 penny stock opportunities right now — prioritize ${hotSectors} sectors based on current momentum`,
    momentum: '5 highest momentum penny stocks with volume surges and strong price action today',
    catalyst: '5 penny stocks with fresh catalysts — FDA, DoD contract, earnings beat, partnership',
    squeeze:  '5 heavily shorted penny stocks primed for a squeeze — high short float, volume building',
    gap:      '5 penny stocks gapping up 10%+ today with volume confirmation — biggest movers',
  };
  const prompt =
...
async function findReplacements(count) {
  ...
  const prompt =
    ...
    `Market Regime: ${marketRegime}. Need high momentum with active catalysts.\n`+
...
setInterval(analyzeAll, 300000);                          // Dual AI every 5 min
